...
function VolunteerRequest(props) {
  ...
  const volRequestApiCall = useCallback(async (event) => {
  ...
  if(volunteerInformation.appList.some(isFind)) {
    requestBtn = (
      <a href='#!' onClick={() => {alert("이미 신청 되어있습니다!");}}><Link to='/VolunteerSearchPage'><Button spanStyle={{margin:"5px"}} btnName='신청'/></Link></a>
    )
  } else {
    requestBtn = (
      <a href='#!' onClick={(event) => {volRequestApiCall(event);}}><Link to='/MyPage'><Button spanStyle={{margin:"5px"}} btnName='신청'/></Link></a>
    )
  }
    ...
    <div className='volunteerRequest-body'>
      ...
      <div>
        <div className='volunteerRequest-body-buttonBox'>
          {requestBtn}
          <a href='#!'><Link to='/VolunteerSearchPage'><Button spanStyle={{margin:"5px"}} btnName='취소'/></Link></a>
        </div>
      </div>
    </div>
  );
