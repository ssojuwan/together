...
function MyInformation(props) {
  ...
  const volRequestCall = useCallback(async () => {
    ...
    try {
      ...
      if(vol_data != null) { // 데이터 성공
        ...
        let intTime = 0;
        vol_data.map((data) => {
          if(new Date() < new Date(data.volEndTime)) {
            wait.push(data);
          } else {
            complete.push(data);
  ...
  /** 신청한 봉사를 취소하는 메소드 */
  const cancelVolApiCall = useCallback(async (event, volId) => {
      ...
      <>
        <tbody>
          <tr>
            ...
            <td className='myInformation-personalInformationBox-table-tbody-td'>{checkGrade(volComplete.length, volTime)}</td>
            <th className='myInformation-personalInformationBox-table-tbody-th'>생년월일</th>
            <td className='myInformation-personalInformationBox-table-tbody-td'>{dayjs(informationList.serBirth).format('YYYY/MM/DD')}</td>
            <th className='myInformation-personalInformationBox-table-tbody-th'>전화번호</th>
            <td className='myInformation-personalInformationBox-table-tbody-td'>{informationList.serMobile}</td>
          </tr>
          <tr>
            <th className='myInformation-personalInformationBox-table-tbody-th'>이름</th>
            <td className='myInformation-personalInformationBox-table-tbody-td'>{informationList.serName}</td>
            <th className='myInformation-personalInformationBox-table-tbody-th'>성별</th>
            <td className='myInformation-personalInformationBox-table-tbody-td'>{informationList.serGender}</td>
            <th className='myInformation-personalInformationBox-table-tbody-th'>이메일</th>
            <td className='myInformation-personalInformationBox-table-tbody-td'>{informationList.serEmail}</td>
          </tr>
        </tbody>
      </>
    )
          ...
          {volWait.map(data => (
            <>
              <tr className='myInformation-volunteerRecordBox-tableWaiting-tbody-tr' key={data.volId}>
                  ...
                  <td className='myInformation-volunteerRecordBox-tableWaiting-cancelBtn'><Button customClickEvent={(event) => {cancelVolApiCall(event, data.volId);}} btnStyle={{margin:"0px"}} spanStyle={{margin:"5px"}} btnName='취소'/></td>
              </tr>
            </>
          ))}
