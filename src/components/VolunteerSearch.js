...
function VolunteerSearch(props) {
    ...
    const [volunteerNameItem, setVolunteerNameItem] = useState(""); // 이름
    const [typeItem, setTypeItem] = useState(""); // 분야
    const [volunteerTitleItem, setVolunteerTitleItem] = useState(""); // 제목
    const [volunteerSearchPostData, setVolunteerSearchPostData] = useState(); // 서버로 보낼 데이터 합친것
    ...
    const searchApiCall = useCallback(async () => {
        ...
        try {
            ...
            // 봉사를 검색하는 통신
            await axios.get('/together/find/all/volunteer', {params:{orgName:volunteerNameItem, volTitle:volunteerTitleItem, volType:typeItem}})
                .then((response) => {
    ...
    }, [setTotalPage, volunteerSearchPostData, currentPage, totalPage])

    /** 초기화 버튼 : 검색 초기화 */
    const resetBtn = useCallback(() => {
        window.location.reload();
    }, [setVolunteerNameItem, setTypeItem, setVolunteerTitleItem])

    const orgNameChange = (e) => {
        setVolunteerNameItem(e.target.value);
    }

    const volTitleChange = (e) => {
        setVolunteerTitleItem(e.target.value);
    }
    ...
    useEffect(() => {
        setVolunteerSearchPostData([
            { orgName:volunteerNameItem, volTitle:volunteerTitleItem, volType:typeItem }
        ])
    }, [volunteerNameItem, typeItem, volunteerTitleItem])
                ...
                <div className='volunteerSearch-searchBox'> {/* 세부 검색 */}
                    ...
                    <table className='volunteerSearch-searchTable'>
                        <tr className='volunteerSearch-searchBox-tr'>
                            <td><span className='volunteerSearch-searchBox-Text'>기관 이름</span></td>
                            <td><input onChange={orgNameChange} value={volunteerNameItem || ""} className='volunteerSearch-searchBox-area-inputText'/></td>
                        </tr>

                        <tr className='volunteerSearch-searchBox-tr'>
                        <td><span className='volunteerSearch-searchBox-Text'>봉사 제목</span></td>
                            <td><input onChange={volTitleChange} value={volunteerTitleItem || ""} className='volunteerSearch-searchBox-area-inputText'/></td>
                        </tr>
                    ...
                    <div className='volunteerSearch-searchBox-button'>
                        <a href="#!" onClick={() => searchApiCall()}><Button spanStyle={{margin:"5px"}} btnName='검색'/></a>
                        <a href="#!" onClick={() => resetBtn()}><Button spanStyle={{margin:"5px"}} btnName='초기화'/></a>
                    </div>
                </div>
            </div>
