    ...
    SerInformComponent = (
      <>
        {serList && serList.map(data => {
          return <>
            <span key={data.serId}>
              <div>이름: {data.servant.serName}</div>
              <div>생년월일: {data.servant.serBirth}</div>
              <div>전화번호: {data.servant.serMobile}</div>
            </span>
          </> 
        })}
      </>
          ...
          <div className="CurrentRecruit_dashboard">
            <div className="CurrentRecruit_detail">
              <div className="CurrentRecruit_detailList1">
                <div>모집 중인 봉사 목록 조회</div>
                <div className="CurrentRecruit_volList">
                  <div className="CurrentRecruit_tableBox_div"> {/* 모집 중인 봉사 테이블 자리 */}
                    <table className="CurrentRecruit_tableList">
                      <thead className="CurrentRecruit_thead">
                        <tr className="CurrentRecruit_tr" style={{ display: 'inline-block' }} >
                          <th className="CurrentRecruit_tableListIdx">번호</th>
                          <th className="CurrentRecruit_tableListIdx">제목</th>
                          <th className="CurrentRecruit_tableListIdx">유형</th>
                          <th className="CurrentRecruit_tableListIdx">모집기간</th>
                            ...
                            <tr key={idx}>
                              ...
                              <td>
                                <input
                                  ...
                                  type="button"
                                  className="result_btn"
                                  onClick={(event) => setRequestPeople(rowData)}
                                  value="신청자 목록"
                                />
                              </td>
                            </tr>
              ...
              <div className="CurrentRecruit_detailList1">
                <div>신청자 목록 조회</div>
                <div className="CurrentRecruit_serList">
                  {SerInformComponent}
                </div>
              </div>
            </div>
